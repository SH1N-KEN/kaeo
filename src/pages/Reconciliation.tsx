  ...
  const renderRecordDetails = (res: ReconciliationMatchResult) => {
    return (
      <div className="bg-[var(--surface-muted)] border border-border/80 rounded p-4 text-xs space-y-4 max-w-4xl mx-auto">
        ...
        {/* AI Exception Resolver — Libby handles investigation via open-ask-libby event */}
        {(() => {
          ...
          const bankTx = res.bankRecord?.transaction ?? null;

          return (
            <div className="border-t border-border/30 pt-4 mt-4 text-left">
              <AskLibbyButton
                label="Investigate with Libby"
                query={`Investigate this ${res.decision.status.toLowerCase()} reconciliation exception${procTx ? ` — ${procTx.description}` : ''}.`}
                reconciliationContext={{
                  exceptionType: res.decision.status as any,
                  processorTxn: procTx ?? null,
                  bankTxn: bankTx,
                  discrepancy: res.decision.reason,
                  amount: res.decision.evidence.amountDifference ?? 0,
                  dateGap: (procTx && bankTx)
                    ? Math.abs(new Date(bankTx.transaction_date).getTime() - new Date(procTx.transaction_date).getTime()) / (1000 * 60 * 60 * 24)
                    : 0
                }}
              />
            </div>
